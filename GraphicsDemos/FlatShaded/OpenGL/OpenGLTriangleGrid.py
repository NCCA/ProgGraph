...
class MainWindow(QOpenGLWindow):
    ...
    def __init__(self, parent: object = None) -> None:
        ...
        self.setTitle("Blinn-Phong Shading OpenGL (Core Profile)")
    ...
    def initializeGL(self) -> None:
        ...
        ShaderLib.set_uniform("shininess", 32.0)
    ...
    def paintGL(self) -> None:
        ...
        model_matrix = Mat4.rotate_y(self.rotation)
        mvp = self.projection @ self.view @ model_matrix

        ShaderLib.set_uniform("M", model_matrix)
    ...
    def timerEvent(self, event) -> None:
        ...
        if self.animate:
            ...
            if ptr:
                ...
                # also need to re-calculate the normals.
                # we can do this by using the cross product of two vectors on the triangle
                # to create a new normal.
                # We do this for each triangle in the mesh and advance the buffer pointer by 3
                for i in range(0, len(buffer), 3):
                    p1 = Vec3(buffer[i]["x"][0], buffer[i]["y"][0], buffer[i]["z"][0])
                    p2 = Vec3(buffer[i + 1]["x"][0], buffer[i + 1]["y"][0], buffer[i + 1]["z"][0])
                    p3 = Vec3(buffer[i + 2]["x"][0], buffer[i + 2]["y"][0], buffer[i + 2]["z"][0])
                    v1 = p2 - p1
                    v2 = p3 - p1
                    normal = v1.cross(v2)
                    normal.normalize()
                    buffer[i]["nx"] = normal.x
                    buffer[i]["ny"] = normal.y
                    buffer[i]["nz"] = normal.z
                    buffer[i + 1]["nx"] = normal.x
                    buffer[i + 1]["ny"] = normal.y
                    buffer[i + 1]["nz"] = normal.z
                    buffer[i + 2]["nx"] = normal.x
                    buffer[i + 2]["ny"] = normal.y
                    buffer[i + 2]["nz"] = normal.z
